#------------------------------------------------------------------------------
# This script receives video from the HoloLens front RGB camera and detects ArUco marker.
...
if (mode == hl2ss.StreamMode.MODE_2):
    ...
    aruco_dictionary = aruco.getPredefinedDictionary(cv2.aruco.DICT_4X4_100)
    aruco_parameters = cv2.aruco.DetectorParameters()
    aruco_parameters.cornerRefinementMethod = cv2.aruco.CORNER_REFINE_SUBPIX # 子像素级别优化
    ...
    while (enable):
        ...
        intrinsics_pv = hl2ss.create_pv_intrinsics(data.payload.focal_length, data.payload.principal_point)
        ...
        print(data.pose)
        # Detect aruco marker
        Image_temp = data.payload.image
        
        corners, ids, _ = aruco_detector.detectMarkers(Image_temp)
        ...
        # Transform aruco corners to world coordinates
            aruco_to_world = aruco_pose @ hl2ss_3dcv.camera_to_rignode(extrinsics_pv) @ hl2ss_3dcv.reference_to_world(data.pose)
            aruco_reference_world = hl2ss_3dcv.transform(aruco_reference, aruco_to_world)
        # Visualize detected markers
        Image_temp = aruco.drawDetectedMarkers(Image_temp, corners, ids)
        cv2.imshow('Video', Image_temp)
        cv2.waitKey(1)
